// --- DATA MAPPINGS (These stay the same) ---
...
function handleBattleResponse(battleJson) {
  try {
    const { MiSh = [], SpSh = [] } = battleJson.OpponentBoard || {};
    ...
    const calculatorState = parseForCalculator(battleJson);
    ...
    // Set the data in storage for the popup to read on opening
...
// The parseForCalculator and generateCalculatorLink functions remain EXACTLY the same.
// You do not need to change them at all.
function parseForCalculator(battleJson) {
  ...
  const parsePet = (petJson) => {
      ...
      return {
          name: petName,
          attack: (petJson.At?.Perm || 0) + (petJson.At?.Temp || 0),
          health: (petJson.Hp?.Perm || 0) + (petJson.Hp?.Temp || 0),
          exp: petJson.Exp || 0,
          equipment: equipment,
          mana: petJson.Mana || 0,
...
function generateCalculatorLink(calculatorState) {
  const baseUrl = "https://sap-calculator.vercel.app/"; 
